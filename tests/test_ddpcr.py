...
def test_ddpcr_examples():
    ...
    assert all(
        c in df.columns
        for c in ["well", "HEX", "FAM", "HEX_target", "FAM_target", "sample_description_1"]
    )
    ...
    df = ddpcr.load_ddpcr(
        "tests/sample_data/Rogi2-Clybl-v3-monoclones-5-24_20241204_100204_335.ddpcr"
    )
    assert all(
        c in df.columns
        for c in ["well", "HEX", "FAM", "HEX_target", "FAM_target", "sample_description_1"]
...
def test_no_metadata():
    ...
    """
    df = ddpcr.load_ddpcr(
        "tests/sample_data/2025.07.18-ddPCR.TANGLEs_20250718_123154_646.ddpcr",
    ...
    df = ddpcr.load_ddpcr(
        "tests/sample_data/ABA_ddPCR_2_20251112_20251112_151023_656.ddpcr", extract_metadata=False
    )
    assert all(c in df.columns for c in ["well", "HEX", "FAM"])
    assert all(c not in df.columns for c in ["HEX_target", "FAM_target", "sample_description_1"])
...
def test_yaml():
    ...
    assert all(
        c in df.columns
        for c in ["well", "HEX", "FAM_target", "sample_description_1", "cell_line", "fam_target"]
    )
...
def test_invalid_yaml(tmp_path: Path):
    ...
    with pytest.raises(ddpcr.YamlError):
        _ = ddpcr.load_ddpcr(
            "tests/sample_data/2025.07.18-ddPCR.TANGLEs_20250718_123154_646.ddpcr", "wells"
        )
