...
def test_default_regex(tmp_path: Path):
    ...
    df_manual = pd.DataFrame(data,columns=['condition','well','population','channel1','channel2'])
...
def test_valid_custom_regex(tmp_path: Path):
    ...
    df_manual = pd.DataFrame(data,columns=['condition','well','dox','population','channel1','channel2'])
    print(df)
    print(df_manual)
    assert df.equals(df_manual)
